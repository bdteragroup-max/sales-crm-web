    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden relative custom-scrollbar flex flex-col h-full bg-gray-50/50 pt-16 md:pt-0">
        <div className="flex-1 overflow-y-auto">
          <NewProjectClient users={JSON.parse(JSON.stringify(users))} jobs={JSON.parse(JSON.stringify(jobs))} currentUserId={user.id} currentUserRole={user.role} initialJobId={initialJobId} />
        </div>
      </main>
    </div>
  )
