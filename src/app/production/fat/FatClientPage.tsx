...
import FatTestModal from './FatTestModal';

export default function FatClientPage({ jobs, currentUser }: { jobs: any[], currentUser: any }) {
  const [selectedJob, setSelectedJob] = useState<any | null>(null);
  const [activeTab, setActiveTab] = useState<'pending' | 'passed'>('pending');

  const pendingJobs = jobs.filter(job => !job.fatReport || job.fatReport.fatStatus === 'Pending' || job.fatReport.fatStatus === 'Failed');
  const passedJobs = jobs.filter(job => job.fatReport?.fatStatus === 'Passed');
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4 sm:mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-gray-900">ทดสอบการทำงาน (FAT)</h1>
          <p className="text-gray-500 font-medium">Factory Acceptance Test สำหรับตู้ที่ผ่าน QC แล้ว</p>
        </div>
      </div>
      ...
      <div className="flex gap-4 border-b border-gray-200 mb-6">
        ...
        >
          รอทดสอบ FAT ({pendingJobs.length})
        </button>
        <button
        ...
        >
          ประวัติการทดสอบ ({passedJobs.length})
        </button>
      </div>
      ...
      {displayedJobs.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <h3 className="text-lg font-black text-gray-900 mb-1">
            {activeTab === 'pending' ? 'ไม่มีงานรอทดสอบ FAT' : 'ไม่มีประวัติการทดสอบ FAT'}
          </h3>
          <p className="text-gray-500 font-medium text-sm">
            {activeTab === 'pending' ? 'ตู้ที่ผ่าน QC แล้วจะมาปรากฏที่นี่' : 'ยังไม่มีตู้ที่ผ่าน FAT'}
          </p>
        </div>
      ) : (
          ...
          {displayedJobs.map((job) => (
            ...
            >
              <div className="p-5 border-b border-gray-50 flex-1">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`text-[10px] px-2 py-1 rounded-full font-bold ${
                    job.fatReport?.fatStatus === 'Failed' ? 'bg-red-100 text-red-700' : 
                    job.fatReport?.fatStatus === 'Passed' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'
                  }`}>
                    {job.fatReport?.fatStatus === 'Failed' ? 'ไม่ผ่าน FAT' : 
                     job.fatReport?.fatStatus === 'Passed' ? 'ผ่าน FAT' : 'รอทดสอบ'}
                  </span>
                </div>
                
                <div className="space-y-2 mt-4 text-xs font-medium text-gray-600">
                  <div className="flex justify-between">
                    <span>ผู้ตรวจสอบ QC:</span>
                    <strong className="text-gray-900">{job.qcReport?.qcInspectorName || '-'}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span>ประเภทตู้:</span>
              ...
              <div className="bg-gray-50 p-3 flex justify-between items-center text-sm font-bold text-gray-600 group-hover:text-red-600 group-hover:bg-red-50 transition-colors">
                <span>เปิดดูรายละเอียด FAT</span>
