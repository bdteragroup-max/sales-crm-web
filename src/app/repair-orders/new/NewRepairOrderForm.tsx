...
export default function NewRepairOrderForm({ companies = [], users = [], currentUser }: { companies: any[], users: any[], currentUser?: any }) {
  ...
  // Default receiver name to the currently logged in user's full name if found in the users list
  const currentFullUser = currentUser ? users.find((u: any) => u.id === currentUser.id) : null;
  ...
  const [formData, setFormData] = useState<any>({
    ...
    invoiceNo: '',
    deliveryNoteNo: '',
    receiverName: currentFullUser?.fullName || '',
    senderName: '',
    forwardedBy: '',
    handoverRef: '',
    salesPerson: currentFullUser?.role === 'SALES' ? currentFullUser?.fullName : '',
    symptoms: '',
    settings: '',
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card title="Section 1: ข้อมูลการซ่อม (Repair Information)">
          <div className="space-y-4">
            <InputField name="jobId" label="Document No. :" type="text" readOnly placeholder="(Auto Generated RO-YY-XXXX)" value="" />
