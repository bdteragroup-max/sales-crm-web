...
import { Save, X, Plus, Trash2, Camera, Printer } from 'lucide-react';
import { LoadingButton } from '@/app/components/LoadingButton';
import { createRepairOrder as saveRepairOrder } from '@/app/actions/repairOrders';
...
export default function RepairOrderForm({ companies = [], users = [], initialData, onSuccess }: RepairOrderFormProps) {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    company: 'TERA GROUP',
    customerCompany: '',
    phoneNumber: '', // Repair Phone Number
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>, printAfter: boolean = false) {
    ...
    // Merge phone number back if needed
    const submitData = {
      ...formData,
      phoneNumber: formData.customerPhoneNumber || formData.phoneNumber
    };

    const res = await saveRepairOrder(submitData);

    if (res.success) {
      setMessage('บันทึกข้อมูลใบรับซ่อมเรียบร้อยแล้ว');
      if (printAfter) {
        const generatedJobId = formData.jobId || res.data?.jobId;
        if (generatedJobId) {
          router.push(`/jobs/${generatedJobId}/repair-order?print=true`);
        }
      } else {
        if (onSuccess) {
    ...
    <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <LoadingButton
          ...
          className="bg-white border border-[#ff2301] text-[#ff2301] hover:bg-red-50 px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 shadow-sm transition-all"
        >
          {!isSubmitting && <Save size={18} />}
          {initialData?.id ? 'บันทึกการแก้ไข' : 'บันทึกข้อมูล'}
        </LoadingButton>
        ...
        <LoadingButton
          ...
          className="bg-[#ff2301] hover:bg-red-700 text-white px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-red-200 transition-all"
        >
          {!isSubmittingAndPrint && <Printer size={18} />}
          บันทึก และ พิมพ์ PDF (Save + Print PDF)
        </LoadingButton>
        ...
        {message && (
          <div className="text-sm font-bold px-4 py-2.5 rounded-xl bg-green-50 text-green-700 border border-green-100 ml-auto">
            {message}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card title="Section 1: ข้อมูลการซ่อม (Repair Information)">
          <div className="space-y-4">
            <InputField name="jobId" label="JOB Number :" type="text" readOnly placeholder="(Auto Generated)" value={formData.job?.jobNumber || ''} />
      ...
      <Card title="Section 4: อาการเสีย และ การตั้งค่า (Symptom & Settings)">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-slate-500">อาการเสีย (Symptom) :</label>
              <textarea 
                name="symptoms" 
                rows={4} 
          ...
          <div className="space-y-4">
            <label className="text-sm font-medium text-slate-500 block">การตรวจสอบสภาพ (Checklist) และรูปภาพ :</label>
